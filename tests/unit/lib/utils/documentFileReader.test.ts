...
const mockFileReader = (resultContent: string[], throwError = false) => {
    global.FileReader = class {
        ...
        readAsText() {
            setTimeout(() => { // Simulate asynchronous behavior of FileReader
                if (throwError) {
...
describe('DocumentFileReader', () => {
    ...
    it('should handle FileReader error', async () => {
        ...
        await expect(DocumentFileReader.readFiles(files)).rejects.toEqual(new Error("Failed to read the file"));
    });
});
